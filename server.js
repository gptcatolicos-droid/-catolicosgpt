...
════════════════════════════════════════════════════`;
  ...
  if (wantStream !== false) {
    ...
    // ════════════════════════════════════════════════════════════════
    // ARQUITECTURA V77 — Magisterium STREAMING DIRECTO + Citas + Preguntas
    // ════════════════════════════════════════════════════════════════
    ...
    // 1. SEARCH en paralelo (no bloquea el stream)
    const searchPromise = buscarEnMagisterium(lastUserMsg, 5, modo).catch(e => {
      console.error('[Magisterium Search]', e.message); return [];
    });

    // 2. MAGISTERIUM STREAMING DIRECTO — raw fetch para capturar citations + related_questions
    let magisteriumOk = false;
    let citations = [];
    let relatedQuestions = [];

    try {
      // Construir mensajes para Magisterium
      const magMessages = [...messages.slice(-6)];
      // Inyectar contexto litúrgico como primer mensaje si existe
      if (contextoLiturgico) {
        magMessages.unshift({
          role: 'user',
          content: 'Contexto litúrgico del día:\n' + contextoLiturgico
        });
        magMessages.splice(1, 0, { role: 'assistant', content: 'Entendido, tengo el contexto litúrgico del día.' });
      }

      const magResp = await fetch('https://www.magisterium.com/api/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${process.env.MAGISTERIUM_API_KEY}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          ...
          messages: magMessages,
          ...(modo !== 'auto' ? { mode: modo } : {})
        }),
        signal: AbortSignal.timeout(30000)
      });

      if (!magResp.ok) throw new Error('Magisterium HTTP ' + magResp.status);

      // Parsear SSE stream de Magisterium
      const reader = magResp.body;
      let sseBuffer = '';
      let fullMagText = '';

      for await (const chunk of reader) {
        sseBuffer += typeof chunk === 'string' ? chunk : new TextDecoder().decode(chunk);
        ...
        for (const line of lines) {
          ...
          try {
            ...
            // Citas y preguntas relacionadas vienen en el último chunk
          ...
          } catch(pe) { /* chunk parcial, ignorar */ }
        }
      }

      magisteriumOk = fullMagText.length > 30;
      if (magisteriumOk) {
        console.log('[Chat V77] ✅ Magisterium streaming OK:', fullMagText.length, 'chars,', citations.length, 'citas,', relatedQuestions.length, 'preguntas');
      }

    } catch(magErr) {
      console.error('[Chat V77] Magisterium streaming falló:', magErr.message);
    }

    // 3. Si Magisterium no respondió → fallback a Anthropic
    if (!magisteriumOk) {
      try {
        console.log('[Chat V77] Fallback Anthropic...');
        const magSearchResultsRaw = await searchPromise;
        const magSearchResults = Array.isArray(magSearchResultsRaw)
          ? magSearchResultsRaw
          : (magSearchResultsRaw?.results || magSearchResultsRaw?.data || []);
        const fuentesBusqueda = formatearFuentesBusqueda(magSearchResults);

        let fallbackSystem = systemPrompt;
        if (fuentesBusqueda.length > 50) {
          fallbackSystem += `\n\nFUENTES DE MAGISTERIUM:\n${fuentesBusqueda}\n\nUsa estas fuentes. Cita con referencia exacta.`;
        }
        if (contextoLiturgico) {
          fallbackSystem += `\n\n${contextoLiturgico}`;
        }
        ...
        console.error('[Chat V77] Anthropic fallback falló:', e2.message);
        res.write(`data: ${JSON.stringify({ error: 'Error al conectar con la IA: ' + e2.message })}\n\n`);
      }
    }

    // 4. Enviar CITAS verificables de Magisterium
    if (citations.length > 0) {
      res.write(`data: ${JSON.stringify({
        citations: citations.map(c => ({
          citedText: c.cited_text || '',
          heading: c.cited_text_heading || null,
          document: c.document_title || 'Documento',
    ...
    // 5. Enviar PREGUNTAS RELACIONADAS
    if (relatedQuestions.length > 0) {
      res.write(`data: ${JSON.stringify({ relatedQuestions })}\n\n`);
    }

    // 6. Enviar fuentes de búsqueda (si hay)
    const magSearchResultsFinal = await searchPromise;
    const searchResults = Array.isArray(magSearchResultsFinal)
      ? magSearchResultsFinal
      : (magSearchResultsFinal?.results || magSearchResultsFinal?.data || []);
    ...
    // No streaming — Magisterium responde con citations
    try {
      const magResp = await fetch('https://www.magisterium.com/api/v1/chat/completions', {
        method: 'POST',
        headers: {
      ...
      const magData = await magResp.json();
      const reply = magData.choices?.[0]?.message?.content || '';
      const citations = magData.citations || [];
      const relatedQuestions = magData.related_questions || [];

      if (reply.length > 50) {
        res.json({ reply, citations, relatedQuestions });
      } else {
        // Fallback Anthropic
